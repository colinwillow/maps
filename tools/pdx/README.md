...
everything it writes lands in `public/pdx/data/`.

```sh
python3 -m pip install pyarrow shapely mapbox_earcut numpy Pillow
python3 tools/pdx/fetch_city.py     # ~6 MB of parquet, cached
python3 tools/pdx/bake.py           # ~50 s -> 100 chunks, 5.2 MB
python3 tools/pdx/test_geo.py       # the projection's directions
```
...
```
fetch_city.py   Overture S3  ->  tools/pdx/.cache/*.parquet
bake.py
...
their footprint, and `public/pdx/game/ground.js` re-implements exactly the same
...
* **A bridge is the one road that must not follow the ground, and solving it is
  ...
  * *Keyed on coincident coordinates.* Right almost everywhere and catastrophic
  ...
  1.5% of drivable bridge segments are over 15%, and `tests/pdx-data.test.mjs`
